...
class ReconstructionLoss_Stage1(torch.nn.Module):
    ...
    def _forward_generator(self,
                           target_codes: torch.Tensor,      # bsz 10 16 16
                           reconstructions: torch.Tensor,   # bsz 10 16 16
                           quantizer_loss: Mapping[Text, torch.Tensor],
                           ) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        reconstructions = reconstructions.contiguous()
        reconstruction_loss = F.smooth_l1_loss(reconstructions, target_codes, reduction='mean')
