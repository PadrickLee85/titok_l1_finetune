...
from modeling.titok import TiTok, ConvVQModel
...
def create_pretrained_tokenizer(config, accelerator=None):
    # One stage
    # Pretrained Tokenizer is a no_require_parms_grad model
    pretrianed_tokenizer = ConvVQModel(config.model.pretrained_lfq_model)
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if model_type == "titok":
        model_cls = TiTok    # TiTok was for 32 tokens encode/quantize/decode  ;  output shape:[bsz,10,16,16]
        loss_cls = ReconstructionLoss_Stage1   # Directly for Latent Loss
    ...
    #NOTE : maybe need to change the ckpt to finetune_titok_version4's.
    model_weight = torch.load("/root/qingfeli/titok_finetune_version1/TiTok/tokenizer_titok_l32.bin", map_location="cpu")
    # Only remove the pixel part of query token and ffn of decoder
    new_model_weight = {}
    for k, v in model_weight.items():
        if 'pixel_' not in k and 'decoder.ffn' not in k:
            new_model_weight[k] =  v
    
    msg = model.load_state_dict(new_model_weight, strict=False)
    logger.info(f"loading weight from msg: {msg}")
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        # do need proxy_features and need pretrained_tokenizer
        pretrained_tokenizer.eval()
        proxy_features = pretrained_tokenizer.encode(images) # bsz 10 16 16

        with accelerator.accumulate([model, loss_module]):
            if model_type == "titok":
                decoded, extra_results_dict = model(images)   # decoded: shape[bsz 10 16 16], treat it as what maskbit encoder gets
...
def eval_reconstruction(
    ...
    for batch in eval_loader:
        ...
        if model_type == "titok":
            decoded, model_dict = local_model(images)  # the output of model is just [bsz, 10, 16, 16]
        elif model_type == "tatitok":
        ...
        with torch.no_grad():
            reconstructed_images = pretrained_tokenizer.decode(decoded)   # tokenizer decoded the decoded into images 
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    if model_type == "titok":
        decoded = accelerator.unwrap_model(model).decode(enc_tokens)  # titok model's output is just the decoded feature [bsz,10,16,16]
    elif model_type == "tatitok":
        reconstructed_images = accelerator.unwrap_model(model).decode(enc_tokens, text_guidance) # bsz c 16 16
        
    with torch.no_grad():
        reconstructed_images = pretrained_tokenizer.decode(decoded)  # after tokenizer, it was decoded to images.
